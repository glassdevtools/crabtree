...
type DownloadAction = {
  href: string;
  label: "Download" | "All Downloads";
};

const defaultDownloadAction: DownloadAction = {
  href: CLIENT_DOWNLOAD_PAGE_PATH,
  label: "All Downloads",
};
...
const HomePage = () => {
  ...
  useEffect(() => {
    ...
    const setAutoDetectedDownloadAction = async () => {
      try {
        const autoDetectedDownloadUrl = await getAutoDetectedDownloadUrl();
        if (didCancel || autoDetectedDownloadUrl === null) {
          return;
        }

        setDownloadAction({
          href: autoDetectedDownloadUrl,
          label: "Download",
        });
      } catch {
        if (!didCancel) {
          setDownloadAction(defaultDownloadAction);
        }
      }
    };

    setAutoDetectedDownloadAction();
    ...
    <main className="page">
      ...
      <section className="hero" aria-labelledby="hero-title">
        ...
        <div className="hero__actions">
          <a className="button button--primary" href={downloadAction.href}>
