...
- Git graph, branches, commits, and worktrees: Codex app-server `command/exec` running Git commands.
...
- Checkout a branch or commit in a worktree: Codex app-server `command/exec` running Git commands, then `thread/metadata/update` if needed.
...
- Use for non-archived Codex thread list/read/start/resume/message/status/metadata operations and Git commands through `command/exec`.
