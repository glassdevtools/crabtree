...
exports.readRepoGraphs = exports.readGitChangesOfCwd = void 0;
...
const runGit = async ({ cwd, args }) => {
    const stdout = await (0, simple_git_1.simpleGit)({ baseDir: cwd }).raw(args);
    return { stdout };
};
const readGitText = async ({ cwd, args }) => {
    const { stdout } = await runGit({ cwd, args });
    return stdout.trim();
};
const readNullableGitText = async ({ cwd, args, }) => {
    try {
...
const readIsDirectory = async (path) => {
    try {
        const pathStat = await (0, promises_1.stat)(path);
...
const readIsGitWorkingTree = async ({ cwd }) => {
...
const readDefaultBranch = async ({ root }) => {
...
const readRepoSeedForThread = async ({ thread }) => {
...
const readRepoSeeds = async ({ threads }) => {
    const repoSeedOfKey = {};
    const repoSeedOfCwd = {};
...
const parseBranchReference = (value) => {
...
const readWorktrees = async ({ repoSeed, threads, }) => {
    ...
    const pushWorktree = () => {
        ...
        const threadIds = threads
            .filter((thread) => thread.cwd === path || thread.cwd.startsWith(`${path}/`))
...
const readGitChangeSummary = async ({ cwd }) => {
...
const readGitBranchTagChanges = async ({ repoSeed, }) => {
    ...
    const remoteShaOfBranch = {};
    const branchTagChanges = [];
    const originPrefix = "origin/";
...
const readGitChangesOfCwd = async ({ threads, repos, }) => {
    const gitChangesOfCwd = {};
    const gitErrors = [];
    const isCwdRead = {};
    const cwds = threads
...
exports.readGitChangesOfCwd = readGitChangesOfCwd;
const readCommits = async ({ repoSeed, threads, worktrees, }) => {
    const format = `%H${FIELD_SEPARATOR}%P${FIELD_SEPARATOR}%D${FIELD_SEPARATOR}%an${FIELD_SEPARATOR}%ad${FIELD_SEPARATOR}%s`;
    const threadIdsOfSha = {};
    const localBranchesOfSha = {};
    const commits = [];
    ...
    for (let skip = 0;; skip += COMMIT_PAGE_SIZE) {
        ...
        for (const line of stdout.split("\n")) {
            ...
            const [sha, parentsText, refsText, author, date, subject] = line.split(FIELD_SEPARATOR);
            if (sha === undefined ||
...
const readRepoGraphs = async ({ threads, }) => {
    const repoSeeds = await readRepoSeeds({ threads });
    const repos = [];
    const warnings = [];
    const gitErrors = [];
    const readMissingParentCount = (commits) => {
        const isCommitOfSha = {};
    ...
    for (const repoSeed of repoSeeds) {
        const threadsInRepo = threads.filter((thread) => repoSeed.threadIds.includes(thread.id));
        try {
            const worktrees = await readWorktrees({
                repoSeed,
                threads: threadsInRepo,
            });
            const [commits, branchTagChanges, isShallowRepositoryText] = await Promise.all([
            ...
            if (isShallowRepositoryText === "true") {
                warnings.push(`${repoSeed.root}: Git repository is shallow, so history can only show commits available locally.`);
            }
            if (missingParentCount > 0) {
                warnings.push(`${repoSeed.root}: ${missingParentCount} parent commits are missing from local Git history.`);
            }
        }
        catch (error) {
            const message = error instanceof Error ? error.message : "Unknown Git error.";
...
exports.readRepoGraphs = readRepoGraphs;
